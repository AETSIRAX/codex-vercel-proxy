...
- Identity confuse 开启时，请求体 `prompt_cache_key`、`client_metadata` 中的 Codex 安装 ID、`session-id` 和 `thread-id` 会按当前上游凭据生成稳定替代值；显式 `x-client-request-id`、turn metadata 和 window id 保持透传；响应返回客户端前恢复原值
...
- `version`、`x-codex-turn-state`、`x-codex-turn-metadata`、`x-codex-window-id`、`x-codex-beta-features` 存在时透传
- Identity confuse 开启时，发往上游的 `session-id`、`thread-id` 使用混淆后的稳定标识；代理默认生成的 `x-client-request-id` 会随 `thread-id` 一起替换，显式传入的 request id、turn metadata 和 window id 保持透传
