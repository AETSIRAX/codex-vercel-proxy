import { userAgent, type AppEnv } from "./env.js";
...
const CHATGPT_RATE_LIMIT_USAGE_URL = "https://chatgpt.com/backend-api/wham/usage";
const USAGE_LIMIT_ERROR_TYPES = new Set(["usage_limit_reached", "usage_not_included"]);
const LOW_REMAINING_USED_PERCENT = 90;

export function isUsageLimitErrorType(value: string | undefined): boolean {
  return value !== undefined && USAGE_LIMIT_ERROR_TYPES.has(value);
}
...
export function nextResetMillisFromRateLimits(
  ...
  const lowRemaining = windows.filter((window) => window.usedPercent > LOW_REMAINING_USED_PERCENT);
  if (lowRemaining.length === 0) {
    return undefined;
  }
  return Math.max(...lowRemaining.map((window) => window.resetAt ?? 0)) * 1000;
}

function rateLimitUsageUrl(): string {
  return CHATGPT_RATE_LIMIT_USAGE_URL;
}
