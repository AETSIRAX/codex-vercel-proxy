...
import { rankCredentialIdsByAffinity } from "./codex-affinity.js";
...
export class CredentialManager {
  ...
  async selectCredential(options: CredentialSelectionOptions = {}): Promise<SelectedCredential | null> {
    await ensureSchema(this.sql);
    const now = Date.now();
    const exclude = [...new Set(options.excludedIds ?? [])];
    const affinityKey = stringValue(options.affinityKey);
    for (;;) {
      const dbRows = await this.claimCredentialCandidate(now, exclude, affinityKey);
      if (dbRows.length === 0) {
        return null;
      }
  ...
  async refreshRateLimits(credential: SelectedCredential): Promise<RateLimitSnapshot[]> {
    ...
    await this.applyRateLimitCooldownIfNeeded(credential.id, snapshots);
    return snapshots;
  }
  ...
  async refreshCredentials(): Promise<CredentialRefreshSummary> {
    const tokenRefresh = await this.refreshAllTokens();
  ...
  private async refreshAllTokens(): Promise<RefreshSummary> {
    await ensureSchema(this.sql);
    const dbRows = await this.sql<CredentialDbRow[]>`
      SELECT id, label, encrypted_json, disabled, last_error, created_at, updated_at,
             last_used_at, next_retry_at, refresh_lock_until, rate_limits_json, rate_limits_updated_at,
  ...
  private async claimCredentialCandidate(
    now: number,
    excludedIds: string[],
    affinityKey: string | undefined,
  ): Promise<CredentialDbRow[]> {
    if (affinityKey !== undefined) {
      return this.claimAffinityCredentialCandidate(now, excludedIds, affinityKey);
    }
  ...
  private async claimAffinityCredentialCandidate(
    now: number,
    excludedIds: string[],
    affinityKey: string,
  ): Promise<CredentialDbRow[]> {
        ...
        : await this.sql<{ id: string }[]>`
            SELECT id
             ...
             WHERE disabled = FALSE
               ...
               AND id NOT IN ${this.sql(excludedIds)}
          `;
    const rankedIds = await rankCredentialIdsByAffinity(
      affinityKey,
      candidates.map((row) => row.id),
    );
    for (const selectedId of rankedIds) {
      const rows = await this.sql<CredentialDbRow[]>`
        WITH candidate AS (
          SELECT id
            FROM credentials
  ...
  private async normalizeImport(input: unknown): Promise<NormalizedImport> {
    ...
    const idToken = optionalImportString(input, "id_token");
    const accountId = optionalImportString(input, "account_id");
    const email = optionalImportString(input, "email");
  ...
  private retryForResult(input: ReportResultInput): RetryDecision {
    if (input.status === 429 && isUsageLimitErrorType(input.errorType)) {
  ...
  private async applyRateLimitCooldownIfNeeded(
    id: string,
    snapshots: RateLimitSnapshot[],
    now = Date.now(),
  ): Promise<void> {
    const nextRetryAt = nextResetMillisFromRateLimits(snapshots, now);
    if (nextRetryAt === undefined) {
      return;
    }
    await this.sql`
      UPDATE credentials
         SET next_retry_at = CASE
               WHEN next_retry_at IS NULL OR next_retry_at < ${nextRetryAt} THEN ${nextRetryAt}
               ELSE next_retry_at
             END,
             last_error = ${`rate limit remaining is below 10%; cooling down until ${isoTime(nextRetryAt)}`},
             updated_at = ${now}
       WHERE id = ${id}
    `;
  }
...
function optionalImportString(input: Record<string, unknown>, field: string): string | undefined {
  return stringValue(input[field]);
}

function requiredImportString(input: Record<string, unknown>, field: string): string {
  const value = optionalImportString(input, field);
  if (value === undefined) {
