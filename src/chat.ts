...
import { applyCodexIdentityExposeText, type CodexIdentityState } from "./codex-identity.js";
...
export async function proxyChatCompletions(request: Request, env: AppEnv, input: JsonObject): Promise<Response> {
  ...
  try {
    for await (const event of readSseData(upstream.response.body)) {
      const parsed = parseSseJson(applyCodexIdentityExposeText(event.data, upstream.identityState));
      if (parsed) {
